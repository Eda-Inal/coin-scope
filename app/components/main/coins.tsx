...
const Coins: React.FC = () => {
    ...
    const [visibleCoins, setVisibleCoins] = useState<CryptoData[]>([]);
    ...
    useEffect(() => {
        setVisibleCoins(filteredCoins.slice(0, itemsToShow));
    }, [filteredCoins, itemsToShow]);
    ...
    return (
        <div className="flex flex-col gap-2 h-full w-full justify-between">
            {/* Başlık */}
            <div className="flex flex-row justify-between font-semibold p-2 border-b dark:border-b-gray-700 items-center sticky top-0 bg-lightPrimary dark:bg-darkBackground z-10">
                ...
                visibleCoins.map((coin, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span onClick={() => handleOpenModal(coin)} className="w-6 h-6 sm:w-8 sm:h-8 rounded-full mr-4 cursor-pointer text-left">
                                <Image
                                    src={coin.image}
                                    alt={coin.name}
                                    layout="intrinsic"
                            ...
                            <span onClick={() => handleOpenModal(coin)} className="flex-[1.2] cursor-pointer text-left font-semibold">
                                {coin.name}
                            </span>
                            <span onClick={() => handleOpenModal(coin)} className="flex-[1.2] text-left cursor-pointer">
                                ${coin.current_price.toLocaleString()}
                            </span>
                            <span
                                onClick={() => handleOpenModal(coin)}
                                className={`flex-[1.2] flex cursor-pointer items-center gap-1 font-semibold ${
